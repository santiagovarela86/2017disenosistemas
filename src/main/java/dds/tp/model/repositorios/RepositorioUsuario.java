...
public class RepositorioUsuario {
	...
	public RepositorioUsuario inicializar() {
	...
	public Usuario buscarUsuarioLogueado(String nombre, String password) {
		password=Utils.Hash(password);
		
		EntityManager manager = PerThreadEntityManagers.getEntityManager();
		 Usuario usuario = manager.createQuery("SELECT u FROM Usuario u WHERE u.nombre = :nombre and u.password = :password",Usuario.class)
				.setParameter("nombre", nombre).setParameter("password", password).getResultList().stream().findFirst().orElse(null);
		manager.close();
		return usuario;
	}
